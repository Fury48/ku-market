...
function createSeedState() {
  ...
  function addUser(spec) {
    const user = {
      ...
      profile_image_url: createAvatar({ name: spec.nickname, accent: spec.accent, background: spec.background }),
  ...
  function addPost(spec) {
    ...
    const post = {
      id: counters.posts++,
      post_name: `post-${spec.slug || counters.posts}`,
  ...
  function addLike(post, user, createdHoursAgo) {
    state.post_likes.push({
      id: counters.postLikes++,
      post_id: post.id,
      user_id: user.id,
  ...
  function addComment(post, user, content, createdHoursAgo) {
    state.comments.push({
  ...
  addComment(posts[8], users.dohyun, '오 이 팁 좋네요. 저도 1시 이후 추천합니다.', 12);
